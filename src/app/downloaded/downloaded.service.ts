...
export class DownloadService {
    ...
    public getDownloadedFromUserId(userId: string): Promise<MyAnimes[]> {
        const url = 'http://localhost:3000/' + userId +'/downloaded';
        return this.http.get<MyAnimes[]>(url).toPromise();
    }
    public addDownloaded(id:string, episode: Episode): Promise<User> {
        return this.http.post<User>('http://localhost:3000/' + id +'/downloaded', episode).toPromise();
    }
}